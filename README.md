...
已完成：电源面板、CPU/内存/存储/网络详情、历史曲线、开机自启、应用图标、网络体检、监听端口、SSH 可达性、温度与功耗、登录项审计、电池健康趋势、阈值告警、DMG 分发（[Releases](https://github.com/isaleafa/Vitals/releases)）。
...
**明确不做**（写在这里是为了让它是"决定"而不是"漏项"）：

- **启动项管理**——即在 App 里禁用/卸载某项自启。能做的部分（用户级 launchd 项）本身不需要授权，但会让 Vitals 第一次拥有写操作、并可能弹管理员授权，与"全部只读、不调 sudo"的定位冲突，故不做，只保留**只读审计**。三类对象的实测能力边界（哪类能管、哪类管不了、为什么）记在 `PLAN.md` 附录 28d
- 采样间隔 / 历史保留天数 / 菜单栏显示项这类设置项——没人会调，只增加出错面
