...
**主动不做（2026-09-24 用户决定，别再当漏项）**：**启动项管理** —— 在 App 里禁用/卸载某项自启。能做到的那部分（用户级 launchd 项，零授权）会把 Vitals 从"全部只读、从不提权"变成有写操作、且 `/Library` 那几项要弹管理员授权；用户判断"既然麻烦就不做了"，保持**只读审计**。三类对象（launchd 项 / Open at Login / 内嵌后台任务）的能力边界与实测证据见附录 28d —— 将来若要重提，先读那条。
